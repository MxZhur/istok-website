...
import { TeachingMaterialData, TagData } from '@/types';
...
function formSubmit() {
    if (props.item) {
        form.post(route('admin.teaching_materials.update', props.item.id), {
            preserveState: true,
        });
    } else {
        form.post(route('admin.teaching_materials.store'), {
            preserveState: true,
        });
    }
}
